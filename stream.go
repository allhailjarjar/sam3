...
//Forward tells SAM to forward incoming stream connections destined for the session destination
//to the host and port provided. The caller must set up a TCP listener on the host:port
//before calling this method. The TCP listener will recieve a new connection on the socket
//for every new connection that SAM recieves from I2P
func (s *StreamSession) Forward(host, port string, useSSL bool) error {
	sam, err := NewSAM(s.samAddr)
	if err != nil {
		return err
	}
	...
	conn := sam.conn
	_, err = conn.Write([]byte("STREAM FORWARD ID=" + s.id + " PORT=" + port + " HOST=" + host + " SSL=" + useSSLStr + "\n"))
	if err != nil {
		conn.Close()
		return err
	}
	...
	for scanner.Scan() {
		...
		default:
			conn.Close()
			return errors.New("Unknown error: " + scanner.Text() + " : " + string(buf[:n]))
		}
	}
